...
public class HumanWorkFlowUnitTest extends TestCase
{
  ...
  private String propertiesFileName = "hwf.ut.properties";
  protected boolean verbose = false;
  private String projectDirectory = ".";

  private String predicateStr = "";  
  /**
   * properties file name can be passed as a System variable named "properties.file.name"
   * default value is "hwf.ut.properties"
   */
  public HumanWorkFlowUnitTest()
  {
  }

  private void init()
  {
    propertiesFileName = System.getProperty("properties.file.name", propertiesFileName);
    //projectDirectory   = System.getProperty("project.directory", projectDirectory);
    projectDirectory = "/home/carlgira/files/soa-testing-snippets~soatestingutilities/SOAInstallationTester";


    System.out.println("- " + propertiesFileName +  " - " + projectDirectory );
...
//      break;
  ...
  public void testHumanTask()
  {
    init();
    beforeFetchingTasks();
    List<Task> list = fetchTaskList();
    for(Task task : list){
      setTaskOutcome(task, "APPROVE");
    }
    afterFetchingTasks(list); 
  }
  
  protected void beforeFetchingTasks()
  { }
  
  protected void afterFetchingTasks(List<Task> list)
  {    
    if (list == null || list.size() == 0)
      System.out.println("Predicate not met:\n" + predicateStr);
    assertTrue("Human Task for " + props.getProperty("user.to.check.name") + " not found.", 
  ...
  {
    System.setProperty("oliv.variable", "Whatever"); // Caution: to be set before the constructor!

    System.setProperty("properties.file.name", "hwf.ut.properties");
    System.setProperty("project.directory", "/home/carlgira/files/soa-testing-snippets~soatestingutilities/SOAInstallationTester");
    System.setProperty("verbose", "true");

    HumanWorkFlowUnitTest hwfut = new HumanWorkFlowUnitTest();
    hwfut.testHumanTask();
  }
}
