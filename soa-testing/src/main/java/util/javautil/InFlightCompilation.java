...
//  options.add(clsDir.toString());
    ...
    System.out.print("Compiling (in flight)..."); 
    ...
    for (Diagnostic diagnostic: diagnostics.getDiagnostics())
    {
      ...
      String line = findLine(code, start);
      System.err.println(line + " (" + line.length() + " char.)");
//    int lineOffset = (int)diagnostic.getEndPosition() - (int)diagnostic.getPosition();
      String errorAt = code.substring(start, end);
      int errorOffset = line.indexOf(errorAt);
      String show = "";
    ...
    if (!success.booleanValue())
    {
      ...
      for (Diagnostic diagnostic : diagnostics.getDiagnostics())
      {
        ...
        String mess = diagnostic.getMessage(Locale.ENGLISH);
        compileError += ("\n--- Error in this line ---\n");
        compileError += (isolateLine(code, (int)lineNumber) + "\n");
        compileError += ("--------------------------\n");
...
//      customClassLoader = new CustomClassLoader(cl != null? cl : Thread.currentThread().getContextClassLoader());        
  ...
  private static String findLine(String code, int offset)
  {
    String line = "";
    int end = code.indexOf("\n", offset);
    String tmp = code.substring(0, end);
    int begin = tmp.lastIndexOf("\n") + 1;
    line = tmp.substring(begin);    
    return line;
  }
  
  private static String isolateLine(String code, int ln)
  {
    return code.split("\n")[ln - 1];
  }
  ...
  private static final String SAMPLE_CODE =
    ...
    "  {\n" +
    "    return \"This class was dynamically generated, compiled and run.\";\n" +
    "  }\n" +
    "}";
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    System.out.println("1. Compiling and running new class test.OlivTest:");
...
//  System.out.println("Object is a " + obj.getClass().getName());
    ...
    m = newClass.getMethod("toString", null); // Non Static, returning a String
    Object returnObject = m.invoke(obj, null);
    if (returnObject instanceof String)
      System.out.println("toString() => \"" + (String)returnObject + "\"");
    else
      System.out.println("toString() returned a " + returnObject.getClass().getName() + " ?!");
  ...
  //  "public class extendedTest extends test.MyClass\n" +
      ...
      "  }\n" +
      "}";
    System.out.println("2. Compiling:\n" + newCode);
